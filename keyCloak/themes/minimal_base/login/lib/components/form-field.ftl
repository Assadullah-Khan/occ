    ...
    <#switch type>
        ...
        <#case "password">
            <div class="mb-3">
                <@label for=id labelText=labelText forRequiredField=booleanAttributes?seq_contains("required") />
                <@helperText text=helperTextBefore />
                <div class="form-control" style="position: relative; padding: 0; display: flex; align-items: center">
                    <input type="${type}"
                           id="${id}"
                           name="${name}"
                            <@attributes attributes=otherAttributes booleanAttributes=booleanAttributes />
                           class="form-control ${hasError?then('border border-danger focus-ring focus-ring-danger', '')}"
                           style="flex-grow:2;"
                    />
...
<#macro error for hasError errorMessage="">
    <#if hasError>
        <label for="${for}" class="invalid-feedback">
