...
public class DefaultPhoneProvider implements PhoneProvider {
    ...
    public int sendTokenCode(String phoneNumber,String sourceAddr,TokenCodeType type, String kind){
        logger.info(Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent() && type.equals(TokenCodeType.REGISTRATION));
        logger.info(!(Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent()) && type.equals(TokenCodeType.AUTH));
        if (Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent() && type.equals(TokenCodeType.REGISTRATION)){
            logger.info("exist:" + phoneNumber );
            throw new ForbiddenException(phoneNumber+": Already Exist");

        }else if(!(Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent()) && type.equals(TokenCodeType.AUTH)){
            logger.info("not exist:" + phoneNumber );
            throw new ForbiddenException(phoneNumber+": Doesn't Exist");
        }
    ...
    @Override
    public void sendAction(String phoneNumber, ActionsEnums action) {
        try {
            session.getProvider(MessageSenderService.class, service).sendAction(phoneNumber,action);
            logger.info(String.format("Sent %s code to %s over %s",action.label, phoneNumber, service));
    ...
    @Override
    public int sendTokenCode(String phoneNumber,String sourceAddr,TokenCodeType type, String kind, String language){
        logger.info(Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent() && type.equals(TokenCodeType.REGISTRATION));
        logger.info(!(Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent()) && type.equals(TokenCodeType.AUTH));
        if (Utils.findUserByPhone(session, session.getContext().getRealm(), phoneNumber).isPresent() && type.equals(TokenCodeType.REGISTRATION)){
            logger.info("exist:" + phoneNumber );
        ...
        try {
            session.getProvider(MessageSenderService.class, service).sendSmsMessage(type,phoneNumber,token.getCode(),tokenExpiresIn,kind, language);
            getTokenCodeService().persistCode(token, type, tokenExpiresIn);
