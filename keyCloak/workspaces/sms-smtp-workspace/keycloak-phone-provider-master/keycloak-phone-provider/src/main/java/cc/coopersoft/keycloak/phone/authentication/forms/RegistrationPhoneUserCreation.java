...
public class RegistrationPhoneUserCreation implements FormActionFactory, FormAction {
  ...
  public void success(FormContext context) {
    ...
    if(Email_Regex_Matcher.matches ()){
      ...
      formData.add(UserModel.EMAIL,email);

    }else{
      email = phoneNumber.substring(1)+"@orbit-users.com";
      formData.remove(UserModel.EMAIL);
      formData.add(UserModel.EMAIL,email);
    }

    String username = formData.getFirst(UserModel.USERNAME);
    logger.info("=>>>>>>>>find me "+ formData.getFirst(UserModel.EMAIL));
    ...
    formData.remove(UserModel.FIRST_NAME);
