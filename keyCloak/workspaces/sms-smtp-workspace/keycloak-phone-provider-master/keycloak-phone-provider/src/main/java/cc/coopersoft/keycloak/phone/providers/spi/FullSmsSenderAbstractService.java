...
public abstract class FullSmsSenderAbstractService implements MessageSenderService {
    ...
    public abstract void sendMessage(String phoneNumber, String message, ActionsEnums action) throws MessageSendException;
    public abstract void sendMessage(String phoneNumber, String message, String language, ActionsEnums action) throws MessageSendException;
    ...
    public void sendSmsMessage(TokenCodeType type, String phoneNumber, String code, int expires, String kind)
        ...
        sendMessage(phoneNumber, code,ActionsEnums.SENDTEMPLATENOTIFICATIONS);
    }

    @Override
    public void sendSmsMessage(TokenCodeType type, String phoneNumber, String code, int expires, String kind, String language)
            throws MessageSendException {
        final String defaultMessage = String.format("[%s] - " + type.label + " code: %s, expires: %s minute ",
                realmDisplay, code, expires / 60);
        final String MESSAGE = localizeMessage(type, phoneNumber, code, expires).orElse(defaultMessage);
        sendMessage(phoneNumber, code, language,ActionsEnums.SENDTEMPLATENOTIFICATIONS);
    }
